...
REMEDIATION: dict[str, tuple[str, str, bool]] = {
    ...
    "OracleDeviationTooHigh":         ("Check oracle feed configuration and retry.", "—", False),
    "TimelockNotElapsed":             ("Wait for the timelock to expire before executing.", "—", False),
    "VacationActive":                 ("Merchant is currently on vacation; retry later.", "—", False),
    "EmergencyWithdrawCooldownActive":("Wait for the emergency withdraw cooldown to expire.", "—", False),
    "EmergencyWithdrawNotRequested":  ("Request an emergency withdrawal first.", "—", False),
    "EmergencyWithdrawStateChanged":  ("The emergency withdrawal state changed unexpectedly.", "—", False),
    "EmergencyWithdrawInvalidState":  ("Invalid state for emergency withdrawal.", "—", False),
    "SelfReferralNotAllowed":         ("A subscriber cannot refer themselves.", "—", False),
    "MerchantNotApproved":            ("The merchant has not been approved.", "—", False),
    "WithdrawCapExceeded":            ("Wait for the withdrawal cap window to reset.", "—", False),
    "SchemaVersionMismatch":          ("Ensure the client and contract schema versions match.", "—", False),
    "DisputeOverpay":                 ("Dispute amount exceeds the allowed limits.", "—", False),
    "CooldownActive":                 ("Wait for the cooldown period to end.", "—", False),
    "RenewalWindowClosed":            ("The renewal window has closed; create a new subscription.", "—", False),
    "DelegatedPayerGrantNotFound":    ("No valid delegated payer grant found.", "—", False),
    "DelegatedPayerGrantExpired":     ("The delegated payer grant has expired.", "—", False),
    "DelegatedPayerAmountExceeded":   ("The delegated payment exceeds the granted amount.", "—", False),
    "EscrowNotFound":                 ("No cancellation escrow found for this subscription.", "—", False),
    "EscrowNotReleased":              ("The cancellation escrow release window has not elapsed.", "—", False),
    "ProposalNotFound":               ("No active admin proposal found.", "—", False),
    "ProposalExpired":                ("The admin proposal has expired.", "—", False),
    "InvalidClaimant":                ("Caller is not the designated claimant for the proposal.", "—", False),
    "ProposalAlreadyExists":          ("An active proposal already exists; cancel it first.", "—", False),
    "NoActiveProposal":               ("No active proposal to claim or cancel.", "—", False),
}
# fmt: on
