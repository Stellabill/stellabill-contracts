...
mod serialization_tests {
    use super::*;
    use soroban_sdk::{vec, Val, Symbol};
    use soroban_sdk::xdr::ScVal;
    use soroban_sdk::{TryIntoVal, IntoVal};
    ...
    fn test_subscription_round_trip() {
        ...
        env.as_contract(&contract_id, || {
            let key = Symbol::new(&env, "test_sub");
            env.storage().instance().set(&key, &original);
            
            let retrieved: Subscription = env.storage().instance().get(&key).unwrap();
            
            assert_eq!(original.subscriber, retrieved.subscriber);
            assert_eq!(original.merchant, retrieved.merchant);
            assert_eq!(original.amount, retrieved.amount);
            assert_eq!(original.interval_seconds, retrieved.interval_seconds);
            assert_eq!(original.last_payment_timestamp, retrieved.last_payment_timestamp);
            assert_eq!(original.status, retrieved.status);
            assert_eq!(original.prepaid_balance, retrieved.prepaid_balance);
            assert_eq!(original.usage_enabled, retrieved.usage_enabled);
        });
    }

    #[test]
    fn test_status_variants_round_trip() {
        let env = Env::default();
        env.mock_all_auths();
        let contract_id = env.register(SubscriptionVault, ());
        let statuses = vec![
        ...
        env.as_contract(&contract_id, || {
            for status in statuses.iter() {
                ...
                let key = Symbol::new(&env, "status_test");
    ...
    #[test]
    fn test_subscription_structure_stability() {
        let env = Env::default();
        let sub = Subscription {
            subscriber: Address::generate(&env),
            merchant: Address::generate(&env),
        ...
        let val: Val = sub.into_val(&env);
        let sc_val: ScVal = val.try_into_val(&env).unwrap();

        if let ScVal::Map(Some(map)) = sc_val {
            assert_eq!(map.len(), 8, "Subscription struct should have 8 fields");
            
            let has_key = |key: &str| -> bool {
                let sym = Symbol::new(&env, key);
                let sym_val: Val = sym.into_val(&env);
                let sym_sc_val: ScVal = sym_val.try_into_val(&env).unwrap();
                
                map.iter().any(|entry| entry.key == sym_sc_val)
            };

            assert!(has_key("subscriber"), "Missing field: subscriber");
            assert!(has_key("merchant"), "Missing field: merchant");
            assert!(has_key("amount"), "Missing field: amount");
            assert!(has_key("interval_seconds"), "Missing field: interval_seconds");
            assert!(has_key("last_payment_timestamp"), "Missing field: last_payment_timestamp");
            assert!(has_key("status"), "Missing field: status");
            assert!(has_key("prepaid_balance"), "Missing field: prepaid_balance");
            assert!(has_key("usage_enabled"), "Missing field: usage_enabled");
        } else {
            panic!("Subscription should serialize to an ScVal::Map");
        }
    }
}
        &false,
