...
pub struct Subscription {
    ...
    /// `false`) the subscriber may re-enable auto-renew without re-creating
    /// the subscription, preserving history and metadata.
    pub auto_renew: bool,
...
pub enum Error {
    ...
    // --- Usage Limits (15000-15099) ---
    /// Subscription requires usage limits to be configured before charging.
    UsageLimitsRequired = 15001,
    /// Subscriber is rate-limited (burst protection).
    SubscriberRateLimited = 15002,
    /// Self-referral is not allowed.
    SelfReferralNotAllowed = 15003,
    /// Merchant tag limit exceeded.
    MerchantTagLimitExceeded = 15004,

    // --- Emergency Withdraw (16000-16099) ---
    /// Emergency withdraw is in an invalid state.
    EmergencyWithdrawInvalidState = 16001,
    /// Emergency withdraw was not requested.
    EmergencyWithdrawNotRequested = 16002,
    /// Emergency withdraw state changed unexpectedly.
    EmergencyWithdrawStateChanged = 16003,
    /// Emergency withdraw cooldown is active.
    EmergencyWithdrawCooldownActive = 16004,
}
...
// ── Cancellation Escrow types ──────────────────────────────────────────────

/// Window (in seconds) during which a cancellation escrow may be released.
pub const CANCELLATION_ESCROW_WINDOW_SECS: u64 = 7 * 24 * 60 * 60; // 7 days

/// Cancellation refund escrow record.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Pending subscription transfer intent.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
...
/// Event emitted when a subscription transfer is completed.
#[contracttype]
#[derive(Clone, Debug)]
...
/// Event emitted when a new token is accepted by the vault.
#[contracttype]
#[derive(Clone, Debug)]
...
/// Event emitted when a fee token is configured.
#[contracttype]
#[derive(Clone, Debug)]
...
/// Event emitted when a fee conversion occurs during charge.
#[contracttype]
#[derive(Clone, Debug)]
...
/// Event emitted when a merchant sub-account is created.
#[contracttype]
#[derive(Clone, Debug)]
...
/// Event emitted when funds are withdrawn from a merchant sub-account.
#[contracttype]
#[derive(Clone, Debug)]
...
// ── Oracle Price History Meta ────────────────────────────────────────────────

/// Ring-buffer metadata for oracle price history.
#[contracttype]
#[derive(Clone, Debug, Eq, PartialEq)]
...
// GraceBuyoutEvent and ReferralAttributedEvent are defined earlier in this file.

/// Normalize a raw token amount to the canonical decimal precision.
pub fn normalize_amount(_env: &Env, _token: &Address, raw: i128) -> Result<i128, Error> {
    // For now, return the raw amount unchanged.
    // A full implementation would look up the token's decimals and scale accordingly.
    Ok(raw)
}
