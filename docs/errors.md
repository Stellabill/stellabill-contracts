...
| 4012 | `CooldownActive` | State Transition | `admin.rs`, `test_multi_token_isolation.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 4013 | `NotInGracePeriod` | State Transition | `subscription.rs` | Refresh state; a grace-period buyout is only legal when status == GracePeriod. | — |
...
| 5001 | `InsufficientBalance` | Accounting | `admin.rs`, `dispute.rs`, `lib.rs`, `merchant.rs`, `subscription.rs`, `test.rs`, `test_dispute_matrix.rs`, `test_merchant_full_drain.rs`, `test_merchant_sub_accounts.rs`, `test_recovery.rs`, `test_reentrancy_invariants.rs` | Retry only after subscriber deposits funds via deposit_funds. | FundsDepositedEvent |
...
| 14101 | `RenewalWindowClosed` | Unknown | `lib.rs`, `subscription.rs`, `test_auto_renew.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 15001 | `EscrowNotFound` | Unknown | `dispute.rs`, `lib.rs`, `test_cancellation_escrow.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 15002 | `EscrowNotReleased` | Unknown | `dispute.rs`, `lib.rs`, `test_cancellation_escrow.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 16001 | `EmergencyWithdrawCooldownActive` | Unknown | `subscription.rs`, `test_emergency_withdraw.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 16002 | `EmergencyWithdrawInvalidState` | Unknown | `subscription.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 16003 | `EmergencyWithdrawNotRequested` | Unknown | `subscription.rs`, `test_emergency_withdraw.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 16004 | `EmergencyWithdrawStateChanged` | Unknown | `subscription.rs`, `test_emergency_withdraw.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |
| 17001 | `SelfReferralNotAllowed` | Unknown | `subscription.rs`, `test_referral_self.rs` | ⚠ No remediation documented — add entry to `REMEDIATION` map in script. | — |

> **⚠ Undocumented variants** — the following variants are present in `types.rs` but have no entry in the script's `REMEDIATION` map: `OracleDeviationTooHigh`, `TimelockNotElapsed`, `CooldownActive`, `VacationActive`, `MerchantNotApproved`, `SchemaVersionMismatch`, `DisputeOverpay`, `DelegatedPayerGrantNotFound`, `DelegatedPayerGrantExpired`, `DelegatedPayerAmountExceeded`, `ProposalNotFound`, `ProposalExpired`, `InvalidClaimant`, `ProposalAlreadyExists`, `NoActiveProposal`, `RenewalWindowClosed`, `EscrowNotFound`, `EscrowNotReleased`, `EmergencyWithdrawCooldownActive`, `EmergencyWithdrawInvalidState`, `EmergencyWithdrawNotRequested`, `EmergencyWithdrawStateChanged`, `SelfReferralNotAllowed`. Add them to `scripts/generate_error_table.py` to resolve this warning.
