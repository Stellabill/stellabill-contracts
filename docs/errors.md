...
| 3009 | `OracleDeviationTooHigh` | Invalid Args | `lib.rs`, `oracle.rs`, `test.rs` | Check oracle feed configuration and retry. | — |
...
| 4011 | `TimelockNotElapsed` | State Transition | `admin.rs`, `test.rs` | Wait for the timelock to expire before executing. | — |
...
| 4014 | `VacationActive` | State Transition | `charge_core.rs`, `merchant.rs`, `subscription.rs`, `test_merchant_vacation.rs`, `types.rs` | Merchant is currently on vacation; retry later. | — |
| 4015 | `EmergencyWithdrawCooldownActive` | State Transition | `subscription.rs`, `test_emergency_withdraw.rs` | Wait for the emergency withdraw cooldown to expire. | — |
| 4016 | `EmergencyWithdrawNotRequested` | State Transition | `subscription.rs`, `test_emergency_withdraw.rs` | Request an emergency withdrawal first. | — |
| 4017 | `EmergencyWithdrawStateChanged` | State Transition | `subscription.rs`, `test_emergency_withdraw.rs` | The emergency withdrawal state changed unexpectedly. | — |
| 4018 | `EmergencyWithdrawInvalidState` | State Transition | `subscription.rs` | Invalid state for emergency withdrawal. | — |
...
| 6019 | `SubscriberRateLimited` | Limits | `subscription.rs` | Retry after the per-subscriber rolling 24-hour subscription-creation window resets; throttle limit is configurable. | — |
...
| 6021 | `MerchantTagLimitExceeded` | Limits | `merchant.rs`, `test_merchant_tags.rs` | Reduce the tag list to at most MAX_MERCHANT_TAGS and retry. | — |
| 6022 | `SelfReferralNotAllowed` | Limits | `subscription.rs`, `test_referral_self.rs` | A subscriber cannot refer themselves. | — |
...
| 7004 | `MerchantNotApproved` | Merchant Config | `merchant.rs`, `test_merchant_whitelist.rs` | The merchant has not been approved. | — |
...
| 7007 | `WithdrawCapExceeded` | Merchant Config | `merchant.rs`, `test_merchant_withdraw_cap.rs` | Wait for the withdrawal cap window to reset. | — |
...
| 9102 | `SchemaVersionMismatch` | Schema Migration | `admin.rs`, `test.rs`, `test_config_migration.rs` | Ensure the client and contract schema versions match. | — |
...
| 10007 | `DisputeOverpay` | Dispute | `dispute.rs` | Dispute amount exceeds the allowed limits. | — |
...
| 12001 | `CooldownActive` | Unknown | `admin.rs`, `test_multi_token_isolation.rs` | Wait for the cooldown period to end. | — |
| 12002 | `RenewalWindowClosed` | Unknown | `lib.rs`, `subscription.rs`, `test_auto_renew.rs` | The renewal window has closed; create a new subscription. | — |
| 13001 | `DelegatedPayerGrantNotFound` | Unknown | `subscription.rs`, `test_delegated_payer.rs` | No valid delegated payer grant found. | — |
| 13002 | `DelegatedPayerGrantExpired` | Unknown | `subscription.rs`, `test_delegated_payer.rs` | The delegated payer grant has expired. | — |
| 13003 | `DelegatedPayerAmountExceeded` | Unknown | `subscription.rs`, `test_delegated_payer.rs` | The delegated payment exceeds the granted amount. | — |
| 13004 | `EscrowNotFound` | Unknown | `dispute.rs`, `lib.rs`, `test_cancellation_escrow.rs` | No cancellation escrow found for this subscription. | — |
| 13005 | `EscrowNotReleased` | Unknown | `dispute.rs`, `lib.rs`, `test_cancellation_escrow.rs` | The cancellation escrow release window has not elapsed. | — |
| 14001 | `ProposalNotFound` | Unknown | `admin.rs`, `test_admin_rotation_two_step.rs` | No active admin proposal found. | — |
| 14002 | `ProposalExpired` | Unknown | `admin.rs`, `test_admin_rotation_two_step.rs` | The admin proposal has expired. | — |
| 14003 | `InvalidClaimant` | Unknown | `admin.rs`, `test_admin_rotation_two_step.rs` | Caller is not the designated claimant for the proposal. | — |
| 14004 | `ProposalAlreadyExists` | Unknown | `admin.rs`, `test_admin_rotation_two_step.rs` | An active proposal already exists; cancel it first. | — |
| 14005 | `NoActiveProposal` | Unknown | `admin.rs`, `test_admin_rotation_two_step.rs` | No active proposal to claim or cancel. | — |
